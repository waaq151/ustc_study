...
def model(X, p_keep_conv, p_keep_hidden):
    ...
    w3 = init_weights([3, 3, 64, 64])    # 3x3x32 conv, 128 outputs
    w4 = init_weights([64 * 20 * 8, 1024]) # FC 128 * 4 * 4 inputs, 625 outputs
    w_o = init_weights([1024, MAX_CAPTCHA*len(CHAR_SET)])         # FC 625 inputs, 10 outputs (labels)
    # 偏置
    b = bias_variable([32])
    b2 = bias_variable([64])
    b3 = bias_variable([64])
    ...
    with tf.name_scope('layer3'):
        l3a = tf.nn.relu(conv2d(l2, w3) + b3)                    # l3a shape=(?, 15, 40, 128)
        l3 = max_pool(l3a)                                        # l3 shape=(?, 8, 20, 128)
        l3 = tf.reshape(l3, [-1, w4.get_shape().as_list()[0]])    # reshape to (?, 20480)        8*20*128=20480  将l3平铺到1维张量
...
def train_crack_captcha_cnn():
    ...
    with tf.name_scope('cost'):
        ...
        train_op = tf.train.AdamOptimizer(0.001, 0.9).minimize(cost)                         # 使用RMSProp优化器构建模型
    ...
    with tf.Session() as sess:
        writer = tf.summary.FileWriter("./人工智能实验课/logs/captcha_logs", sess.graph)
